...
class PMO_Events_Module {
	...
	public static function render_event_details_meta_box( $post ) {
		...
		<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
			...
			<div style="grid-column: 1 / -1;">
				<label for="event_contact">
					<strong><?php esc_html_e( 'Contact Information (Optional):', 'pmo-core' ); ?></strong>
				</label>
				<input
					type="text"
					id="event_contact"
			...
			register_rest_field( 'pmo_event', $field, array(
				...
				'update_callback' => function( $value, $post ) use ( $field ) {
					if ( current_user_can( 'edit_post', $post->ID ) ) {
						update_post_meta( $post->ID, '_' . $field, sanitize_text_field( $value ) );
					}
				},
				'schema'          => array(
					'type' => 'string',
	...
	public static function set_custom_columns( $columns ) {
		$columns = array_slice( $columns, 0, 2, true ) +
			array(
				'event_date' => esc_html__( 'Date', 'pmo-core' ),
				'event_time' => esc_html__( 'Time', 'pmo-core' ),
